...
async def listen_for_logs():
    ...
    async with websockets.connect(uri) as ws:
        ...
        while True:
            ...
            print(f"Raw response: {response}")  # Print the raw response to check its structure
            ...
            # If we received an acknowledgment from the server
            if 'result' in log_data:
                print(f"Subscription acknowledged with ID: {log_data['result']}")
                continue  # Skip processing this response, as it's just the subscription confirmation

            # Check if the response contains the expected log entry data
            if 'params' in log_data and 'result' in log_data['params']:
                result = log_data['params']['result']
                value = result.get('value', {})

                # Check if the value contains a 'signature'
                signature = value.get('signature')
                if signature:
                    print(f"New log received for transaction: {signature}")
                    ...
                    print("No signature found in log entry.")
            else:
                print("Log entry does not contain expected data.")
